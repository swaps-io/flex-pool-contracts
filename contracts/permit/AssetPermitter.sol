...
abstract contract AssetPermitter is IAssetPermitter {
    ...
    function usePermit(address from_, uint256 amount_, uint256 deadline_, bytes32 r_, bytes32 vs_) external {
        ...
        try IERC20Permit(address(permitAsset)).permit(from_, address(this), amount_, deadline_, v, r_, s) {} catch {}
    }
    ...
    function useDaiPermit(address from_, bool allowed_, uint256 deadline_, bytes32 r_, bytes32 vs_) external {
        ...
        (bytes32 s, uint8 v) = _unpackVs(vs_);
        try IDaiPermit(address(permitAsset)).permit(from_, address(this), nonce, deadline_, allowed_, v, r_, s) {} catch {}
    }
    ...
    function useUniswapPermit(address from_, uint256 amount_, uint256 deadline_, bytes calldata signature_) external {
        ...
        try IPermit2(PERMIT2).permitTransferFrom(
            PermitTransferFrom({permitted: TokenPermissions({token: address(permitAsset), amount: amount_}), nonce: nonce, deadline: deadline_}),
    ...
    function useSafePermit(address from_, uint256 amount_, bytes calldata signature_) external {
        ...
        IGnosisSafe(from_).execTransaction(address(permitAsset), 0, data, IGnosisSafe.Operation.Call, 0, 0, 0, address(0), payable(0), signature_);
    }

    // Based on OpenZeppelin library (v5.3.0) internal implementation. See "tryRecover(bytes32,bytes32,bytes32)" in "ECDSA.sol".
